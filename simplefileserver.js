...
function getfiles(f = dm + folder) {
  ...
  files.forEach(file => {
    ...
    if (isdir)
      s = "/"
    dir += '<div ' + ((isdir) ? "class=ifolder" : "class=ifile") + '></div><a href="' + encodeURIComponent(file) + s + '?token=' + gentoken() + '">' + file + '</a>' + deleteButton(file) + '<br>'
  })
  return dir;
}
let files = function (dir, resp, mappingstr = "") {
  ...
  resp.end('<html><head>' +
    ...
    '<div class"input-field col s12"><input type=text name=dir id=dir class="col s3" placeholder="Nueva Carpeta" required /></div> <input type=submit class="btn-small" value="Crear" /></form></div><hr>' +    
    out + 
    '<hr></div></html>');
}
