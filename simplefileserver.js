...
let files = function (dir, resp, mappingstr = "") {
  ...
  if (mappingstr != "") {
    out = replaceGlobally(out, dir.substring(0, dir.indexOf(mappingstr) + mappingstr.length + 1), "/" + mapping + mappingstr + "/")
    out = replaceGlobally(out, "//", "/")
  }
  resp.writeHead(200, {
  ...
  resp.end('<html><head>' + favico +
    ...
    '<title>SimpleFile Server</title><h1>SimpleFile Server</h1> <br><hr> ' + out + '</html>');
}
...
let getMapping = function () {
  ...
  let content = ""
  try {
    content = fs.readFileSync(dm + folder + "/" + virtual, 'utf8')
  } catch (error) {
    return
  }
